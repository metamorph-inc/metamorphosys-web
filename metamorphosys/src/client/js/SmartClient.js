...
define( [ 'js/client',
...
], function ( Client, BlobClient, ExecutorClient, Core, PluginManagerBase, PluginResult, ServerInfoClient ) {
    "use strict";
    ...
    var SmartClient = function ( config ) {
        ...
        this.metaTerritoryPattern[ this.ROOT_ID ] = {
            children: 0
        };
        this.metaNodes = {};
        ...
        self.client.connectToDatabaseAsync( null, function ( err ) {
            ...
            self.client.getFullProjectListAsync( function ( err, projectList ) {
                ...
                if ( projectList.hasOwnProperty( projectName ) ) {
                    ...
                    self.initialMessages.push( {
                        message: projectName + ' project exists.',
                        severity: 'debug'
                    } );
                    ...
                    if ( projectJSON ) {
                        self.client.createProjectFromFileAsync( projectName, JSON.parse( projectJSON ),
                            function ( err ) {
                                if ( err ) {
                                    console.error( projectName + ' project import failed.' );
                                    self.initialMessages.push( {
                                        message: projectName + ' project import failed.',
                                        severity: 'error'
                                    } );
                                    callback( err );
                                    return;
                                ...
                                self.initialMessages.push( {
                                    message: projectName + ' project was imported.',
                                    severity: 'info'
                                } );
                        ...
                        console.info( projectName +
                            ' project template was not defined. Creating an empty project.' );
                        self.initialMessages.push( {
                            message: projectName +
                                ' project template was not defined. Creating an empty project.',
                            severity: 'warning'
                    ...
                    if ( event.etype === 'unload' ) {
                        ...
                        while ( len-- ) {
                            ...
                            if ( metaNode ) {
                                ...
                                self.metaTerritoryPattern[ metaObjectIds[ len ] ] = {
                                    children: 0
                                };
                                patternUpdated = true;
                            }
    ...
    SmartClient.prototype.isMetaTypeOf = function ( node, metaNode ) {
        ...
        console.assert( typeof node === 'string' || typeof node === 'object',
            'Node has to be an id (path) [string] or a node object [object].' );
        console.assert( typeof metaNode === 'string' || typeof metaNode === 'object',
            'Node has to be an id (path) or meta type name [string] or a node object [object].' );
                ...
                if ( event.etype === 'load' ) {
                    ...
                    while ( len-- ) {
                        if ( self.isMetaTypeOf( node, types[ len ] ) ) {
                            self.uiTerritories[ terrId ][ node.getId() ] = {
                                children: 1
                            };
                            updateTerr = true;
                        }
