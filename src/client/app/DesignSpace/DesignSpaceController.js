...
define( [ '../../js/DesertFrontEnd',
    'xmljsonconverter'
], function ( DesertFrontEnd, Converter ) {
    'use strict';

    var DesignSpaceController = function ( $scope, $rootScope, $moment, $routeParams, $location, smartClient,
        Chance, growl ) {
        var self = this;
                ...
                if ( event.etype === 'unload' ) {
                    ...
                    if ( event.etype === 'load' ) {
                        ...
                        self.desertFrontEnd.addSimpleListener( self.$scope.id, function ( status ) {
                            ...
                            self.update();
                            if ( status.backFile ) {
                                self.getDesertDesertBackSystem( status.backFile.content, function ( err,
                                    desertBackSystem ) {
                                    var j,
                                        k,
                                        cfg,
                                    ...
                                    for ( j = 0; j < desertBackSystem.Element.length; j += 1 ) {
                                        ...
                                        elemIdToPath[ elem[ '@_id' ] ] = status.idMap[ elem[
                                            '@externalID' ] ];
                                    }
                                    for ( j = 0; j < desertBackSystem.Configuration.length; j += 1 ) {
                                        ...
                                        for ( k = 0; k < cfg.AlternativeAssignment.length; k += 1 ) {
                                            ...
                                            self.$scope.desert.cfgs[ cfg[ '@id' ] ].alternativeAssignments[
                                                elemIdToPath[ altAss[ '@alternative_end_' ] ] ] = {
                                                selectedAlternative: elemIdToPath[ altAss[
                                                    '@alternative_end_' ] ],
                                                alternativeOf: elemIdToPath[ altAss[
                                                    '@alternative_of_end_' ] ]
                                            };
                                        }
                                    }
    ...
    DesignSpaceController.prototype.saveConfigurationSet = function ( name ) {
        ...
        if ( name ) {
            self.smartClient.client.setAttributes( cfgSetPath, 'name', name, '[WebCyPhy] - DesertCfg was named: ' +
                name );
        }
        ...
        for ( key in self.$scope.desert.cfgs ) {
            if ( self.$scope.desert.cfgs.hasOwnProperty( key ) ) {
                ...
                if ( cfg.isSelected ) {
                    ...
                    self.smartClient.client.setAttributes( cfgPath, 'AlternativeAssignments', JSON.stringify( aa,
                            null ),
            ...
            menu: [ {
                ...
                items: [ {
                    ...
                    action: function () {
                        ...
                        if ( Object.keys( self.$scope.desert.cfgs )
                            .length === 0 ) {
                            self.growl.error(
                                'No configurations to save. Did you calculate the design space?' );
                            return;
                        }
