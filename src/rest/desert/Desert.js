...
define(['logManager',
        ...
        'path',
        'child_process',
        'desert/MetaPath'], function (logManager, BlobRunPluginClient, BlobFSBackend, fs, path, child_process, MetaPath) {
    'use strict';
            ...
            self.blobClient.getMetadata(desertConfigHash, function (err, metadata) {
                ...
                self.blobClient.getObject(desertConfigHash, function (err, content) {
                    ...
                    fs.writeFile(inputXml, content, function (err) {
                        ...
                        child_process.exec(cmd, {cwd: runDir}, function (error, stdout, stderr) {
                            var outputXmlPath;
                            ...
                            if (error) {
                                self.logger.error('Desert Execution return with error :' + error.toString());
    ...
    desertRestCreate = function (req, res, next) {
        ...
        hash = url[2];
        jobInfo = {inputHash: hash, status: 'Created', outputHash: null};
